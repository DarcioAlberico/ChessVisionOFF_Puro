...
class ItemEntregueTemSpecTests(unittest.TestCase):
    ...
    def test_a_renumeracao_declarada_vale(self) -> None:
        ...
        o antigo não pode voltar a nomear a MESMA coisa -- que é a colisão que a renumeração
        desfez. Ele pode, sim, nomear outra: liberar o número para quem já o reservava é o
        motivo de a renumeração ter acontecido, e foi o que aconteceu com a S-431, que hoje é
        o item da revisão externa em `SPEC_REVISAO_EXTERNA.md`. Quem garante que ninguém
        nomeia duas coisas com um número é `NumeroDeItemUnicoTests`; aqui basta exigir que o
        antigo não tenha reaparecido no arquivo de onde saiu.
        """
        com_secao: set[int] = set()
        for arquivo, numeros in self.secoes.items():
            if arquivo not in ARQUIVOS_DE_MEDICAO:
        ...
        for antigo, novo in sorted(RENUMERADOS.items()):
            ...
            arquivo_do_novo = {a for a, ns in self.secoes.items() if novo in ns}
            arquivo_do_antigo = {a for a, ns in self.secoes.items() if antigo in ns}
            if arquivo_do_antigo & arquivo_do_novo:
                problemas.append(
                    f"{_rotulo(antigo)} voltou ao arquivo de {_rotulo(novo)} -- "
                    "a renumeração não aconteceu"
                )
...
class NumeroDeItemUnicoTests(unittest.TestCase):
    ...
    **O defeito, e ele é desta casa.** O `ItemEntregueTemSpecTests` acima confere *presença*:
    ...
    `ANALISE_DETECCAO.md`. Corrigido para `S-219` -- e `S-219` colidiu com outra sessão, que
    ...
    **Por que `secoes_por_arquivo` não podia pegar.** Ela devolve `set[int]` por arquivo. Duas
    seções `## S-219` no mesmo arquivo viram um elemento só, e a repetição desaparece antes de
...
class IndiceDoReadmeTests(unittest.TestCase):
    ...
    def test_nenhuma_linha_de_faixa_esta_solta_fora_da_tabela(self) -> None:
        ...
        Aconteceu duas vezes no mesmo dia: uma inserção automática pôs `| S-220 a S-234, S-324 | ... |`
...
"""Campos de `docs/metrics/*.json` que guardam caminho, e só eles (S-219).
...
class CaminhoPublicadoTests(unittest.TestCase):
    """Nenhum relatório publica a raiz do disco de quem mediu (S-219).
