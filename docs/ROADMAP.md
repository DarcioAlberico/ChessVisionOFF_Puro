...
> 2026-08-16), especificadas em [SPEC_FASE14.md](SPEC_FASE14.md) (S-95 a S-142). Elas abrem
...
| a janela abaixo de 600 linhas | **651** (477 de código) | **1.891**, e o arquivo dobrou depois da decomposição — ver S-136. O corte do Tk (S-506) trocou o arquivo vigiado: era o `app_tkinter.py` com 2.327 linhas, é `qt/janela.py` — que subiu de 1.227 para 1.788 ao receber de volta o que o corte tinha deixado sem chamador (estado da janela, aparência, densidade, conjunto de peças, árbitro do `Ctrl+Z`, livros recentes), a 1.805 com o motor das preferências (S-523), e a 1.891 com o duplo clique que leva o diagrama do visualizador à sala de estudo e a detecção ao virar a página (S-68), cada subida registrada na catraca de `test_packaging` |
