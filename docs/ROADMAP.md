...
| a janela abaixo de 600 linhas | **651** (477 de código) | **1.788**, e o arquivo dobrou depois da decomposição — ver S-136. O corte do Tk (S-506) trocou o arquivo vigiado: era o `app_tkinter.py` com 2.327 linhas, é `qt/janela.py` — que subiu de 1.227 para 1.788 ao receber de volta o que o corte tinha deixado sem chamador (estado da janela, aparência, densidade, conjunto de peças, árbitro do `Ctrl+Z`, livros recentes), cada subida registrada na catraca de `test_packaging` |
