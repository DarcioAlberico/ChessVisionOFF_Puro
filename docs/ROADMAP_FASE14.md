...
| 6 | ⚠ | `app_tkinter.py` em 1.891 contra o alvo de 600 (S-31 aberta, com catraca; +65 nas S-324 a S-226, +81 na Fase 37, +21 na S-445, +10 na S-448, +4 na S-512, +8 nas S-518/S-519, +17 na S-523 e +86 com o duplo clique e a detecção ao virar a página (S-68), cada subida com o motivo no `test_packaging`; o corte do Tk da S-506 trocou o arquivo vigiado pelo `qt/janela.py`, o placar caiu de 2.327 para 1.196 e voltou a subir com o que o corte tinha deixado sem chamador) |
...
| **Trocar `models/piece_classifier.pt` pelo controle** — **a premissa virou em 2026-09-01, e a decisão continua sua** | O "não" de 18/08 dizia que a diferença era um diagrama sem FEN, sobre 19 páginas. Sobre as 66 da S-99 fechada, remedidas com o código da S-176, o controle **exportava mais** (0,7913 contra 0,7652) e **não exportava nenhum errado** (exatidão de campo 1,0000 contra 0,9882) — e era esse o argumento para trocar. **Ele não vale mais.** O `piece_classifier.pt` foi retreinado desde então (o checkpoint de 22/08 não existe mais no disco), e a remedição do fecho do corte do Tk (S-506) dá **0,8696 contra os 0,7913 do controle**: hoje é a produção que exporta mais, e lê certo mais diagramas (`exact` 93 contra 89). O preço é `exported_wrong` 2 contra 0, e exatidão de campo 0,9787 contra 1,0000 — a produção exporta doze a mais e erra dois. Trocar por um modelo que exporta menos e nunca erra continua sendo uma escolha defensável; o que mudou é que ela deixou de ser a leitura óbvia dos números. **Não decidi nada aqui** |
