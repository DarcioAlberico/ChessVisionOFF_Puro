...
class CheckpointDescription:
    """Quem é o `.pt` que produziu um número, em campos que cabem num relatório (S-219).
...
def describe_checkpoint(path: Path) -> CheckpointDescription:
    """Lê o `.pt` uma vez e devolve quem ele é. **Nunca levanta** (S-219).
...
def git_worktree_dirty() -> bool:
    """Se a árvore que está rodando tem mudança não commitada (S-219).
